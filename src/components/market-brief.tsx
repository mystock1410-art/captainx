...
export function MarketBriefPanel() {
  ...
  function handleToggle() {
    if (open) {
      // Collapse panel; require fresh form on next open.
      setOpen(false);
      return;
    }
    // Every open must be gated by a fresh lead submission — no session memory.
  ...
  const highlightCTA = !data && !loading && !open;

  return (
    <section className={cn("rounded-sm border bg-card", highlightCTA ? "cta-border" : "border-border")}>
      <button
        type="button"
      ...
      >
        ...
        {loading ? (
          ...
          <span className="ml-auto text-[10px] text-muted-foreground" key={tick}>
            Cập nhật {fmtAgo(data.generated_at)}
          </span>
        ) : (
          <span className="ml-auto inline-flex items-center gap-1.5">
            <span className="cta-dot inline-block h-1.5 w-1.5 rounded-full bg-gold" />
            <span className="cta-blink text-[11px] font-bold uppercase tracking-[0.12em]">
              Click để phân tích
            </span>
          </span>
        )}
        {open ? (
          ...
          <ChevronDown className={cn("h-4 w-4", data || loading ? "text-muted-foreground" : "text-gold cta-blink")} />
        )}
      </button>
          ...
          {data && (
            <div className="space-y-4 p-4 pt-0">
              ...
              {data.catalysts.length > 0 && (
                <div>
                  <h4 className="mb-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-brand">
                    Catalysts chính
                  </h4>
              ...
              {data.risks.length > 0 && (
                <div>
                  <h4 className="mb-2 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-brand">
  ...
  return (
    <div className="rounded-sm border border-border bg-background p-3">
      <div className="mb-2 flex items-center gap-2">
        ...
        <h4 className="text-[11px] font-semibold uppercase tracking-[0.18em] text-brand">{title}</h4>
      </div>
